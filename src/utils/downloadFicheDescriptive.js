...
function generateAndDownloadPdf(stage, etudiant, ecole, encadrant, entreprise) {
    ...
    doc.setFontSize(12);
    doc.text(`Titre du Stage: ${stage.titre}`, 10, 30);
    doc.text(`Description: ${stage.description}`, 10, 40);
    doc.text(`Date de Début: ${stage.dateDebut}`, 10, 50);
    doc.text(`Date de Fin: ${stage.dateFin}`, 10, 60);
    doc.text(`Durée: ${stage.duree}`, 10, 70);
    doc.text(`Localisation: ${stage.localisation}`, 10, 80);
    doc.text(`Montant de Rémunération: ${stage.montantRemuneration}`, 10, 90);
    doc.text(`Statut: ${stage.statut}`, 10, 100);
    doc.text(`Type: ${stage.type}`, 10, 110);

    // Add student information
    doc.text(`Informations sur l'Étudiant:`, 10, 120);
    doc.text(`Nom: ${etudiant.nom}`, 10, 130);
    doc.text(`Prénom: ${etudiant.prenom}`, 10, 140);
    doc.text(`Email: ${etudiant.email}`, 10, 150);
    doc.text(`Téléphone: ${etudiant.tel}`, 10, 160);

    // Add school information
    doc.text(`Informations sur l'École:`, 10, 170);
    doc.text(`Nom de l'École: ${ecole.nomEcole}`, 10, 180);
    doc.text(`Ville: ${ecole.villeEcole}`, 10, 190);
    ...
    if (ecole.logo) {
        try {
            ...
            doc.addImage(logoBase64, 'PNG', 150, 170, 40, 20); // Adjust position and size as needed
        } catch (error) {
    ...
    doc.text(`Informations sur l'Encadrant:`, 10, 210);
    doc.text(`Nom: ${encadrant.nom}`, 10, 220);
    doc.text(`Prénom: ${encadrant.prenom}`, 10, 230);
    doc.text(`Email: ${encadrant.email}`, 10, 240);
    doc.text(`Téléphone: ${encadrant.telephone}`, 10, 250);

    // Add company information
    doc.text(`Informations sur l'Entreprise:`, 10, 260);
    doc.text(`Nom de l'Entreprise: ${entreprise.nomEntreprise}`, 10, 270);
    doc.text(`Ville: ${entreprise.villeEntreprise}`, 10, 280);
    doc.text(`Adresse: ${entreprise.adresseEntreprise}`, 10, 290);
    ...
    if (entreprise.logo) {
        try {
            ...
            doc.addImage(logoBase64, 'PNG', 150, 260, 40, 20); // Adjust position and size as needed
        } catch (error) {
