...
export default function UniversityInternships() {
  ...
  const fetchCompteEcole = async () => {
    try {
      const id = localStorage.getItem('id');
      const compteEcoleResponse = await axiosInstance.get(`/compte-ecoles/${id}`);
      setEntrepriseId(compteEcoleResponse.data.ecoleId);
      fetchInternships(compteEcoleResponse.data.ecoleId);
